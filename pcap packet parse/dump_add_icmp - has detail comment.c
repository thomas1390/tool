...
//ok  if (-1 == fseek (fp, sizeof (struct pcap_file_header) +sizeof (struct pcap_pkthdr), SEEK_SET))
/*因为此时SEEK_CUR已经在(*pkt_p).len的位置，处于EthernetPacket结构的开始处，所以不再需要用fseek移动了。
  if (-1 == fseek (fp, sizeof (struct pcap_pkthdr), SEEK_CUR))
    {
      printf ("2nd fseek error:%s\n", strerror (errno));
    }
*/
//ok fseek (fp, 40, SEEK_SET);
...
//如果不是IP和arp协议，则直接进入下一个 包，不再解包        
	default:
	  //将文件的读写位置fp重置到下一个包的起始位置
	  // 1.这里用的是(*pkt_p).caplen，即抓到的长度
	  // 2.用SEEK_CUR
...
//   icmp = (struct icmpv4_hdr *) (packet+size_ip+offset);
//下面强制类型转换
...
/*
   ...
   printf("Lenght IP: %d\t", ntohs(ip->ip_len));
   printf("TTL: %d\n", ip->ip_ttl);
   */
   printf("ICMP Type: ");
